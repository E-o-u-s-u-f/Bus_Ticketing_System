...
export default function ScheduleSearch() {
  ...
  const fromOptions = useMemo(() => {
    const seen = new Set();
    return allSchedules.map(s => s.fromlocation).filter(f => (seen.has(f) ? false : seen.add(f))).sort();
  }, [allSchedules]);
  ...
  const sameDate = (iso, yyyy_mm_dd) => {
    try { return new Date(iso).toISOString().split("T")[0] === yyyy_mm_dd; } catch { return false; }
  };

  const handleSearch = e => {
    e.preventDefault();
    setMsg(null);
    if (!fromLoc || !toLoc || !date) { setMsg("Please select From, To, and Date."); return; }
    setLoading(true);
    try {
      let filtered = allSchedules.filter(
        row => row.fromlocation.trim().toLowerCase() === fromLoc.trim().toLowerCase() &&
               row.tolocation.trim().toLowerCase() === toLoc.trim().toLowerCase() &&
               sameDate(row.dtime, date)
      );
      if (upcomingOnly) filtered = filtered.filter(row => new Date(row.dtime) >= new Date());
      setSchedules(filtered);
      if (filtered.length === 0) setMsg("No schedules found.");
    } catch (err) { setMsg(err.message || "Search failed"); }
    finally { setLoading(false); }
  };

  const clearSearch = () => { setFromLoc(""); setToLoc(""); setDate(""); setMsg(null); setSchedules(allSchedules); };
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-950 to-black p-4 sm:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl p-6 mb-6">
          <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <label className="block">
              ...
              <select className="mt-1 w-full rounded-xl bg-white text-black border border-white/10 px-4 py-3 outline-none" value={fromLoc} onChange={e => { setFromLoc(e.target.value); setToLoc(""); }} required>
            ...
            <label className="block">
              ...
              <select className="mt-1 w-full rounded-xl bg-white text-black border border-white/10 px-4 py-3 outline-none" value={toLoc} onChange={e => setToLoc(e.target.value)} required disabled={!fromLoc}>
            ...
            <label className="block">
              ...
              <input type="date" className="mt-1 w-full rounded-xl bg-white text-black border border-white/10 px-4 py-3 outline-none" value={date} onChange={e => setDate(e.target.value)} required />
            </label>
            <div className="md:col-span-4 flex flex-col sm:flex-row items-start sm:items-center gap-3 mt-2">
              <label className="inline-flex items-center gap-2 text-slate-300">
                <input type="checkbox" className="accent-cyan-400 h-4 w-4" checked={upcomingOnly} onChange={e => setUpcomingOnly(e.target.checked)} /> Upcoming only
              </label>
              <div className="ml-auto flex gap-3">
                ...
                <button type="submit" className="rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 text-white font-medium px-5 py-2.5 hover:opacity-95">Search</button>
              </div>
            </div>
          </form>
        ...
        <div className="space-y-3">
          {loading ? <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl p-6 text-slate-300">Loading schedules…</div> :
              ...
              <div key={row.scheid} className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl p-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="text-slate-300/90 text-sm">{dt.toLocaleString()} • Bus: {row.busnumber} ({row.bustype}) {row.drivername ? `• Driver: ${row.drivername}` : ""}</div>
                  </div>
          ...
          }) : <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl p-6 text-slate-300">No schedules to show.</div>}
        </div>
      </div>
    </div>
  );
